...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST)) {
    ...
    $file_dest = $upload_folder . $file_newname;  // Using the correct upload folder path

    // If no required fields are filled, display an error
    if (empty($name) || empty($desc) || empty($price) || empty($cat)) {
        $upload_err = "Name, Description, Price and Category cannot be empty!!";
    } else {
        // Check if the file is uploaded
        if (empty($file_tmpname) || !isset($file)) {
            // No file uploaded, proceed without imgPath update
        ...
        if (!isset($err_msg)) {
            ...
            if (isset($imgPath)) {
                $stmt->bindParam(':imgPath', $imgPath);  // Only bind imgPath if it's set
            }
